...
export const borrowBook = catchAsync(async (req: Request, res: Response) => {
  ...
  if (!mongoose.Types.ObjectId.isValid(bookId)) {
    return res
      ...
      .json(
        errorResponse(
          "Invalid Book ID",
          "The provided book ID is not a valid MongoDB ObjectId."
        )
      );
  }
    ...
    return res
      ...
      .json(
        errorResponse(
          ...
          `Only ${book.copies} copies of "${book.title}" are available.`
        )
      );
  }
  ...
  const responseData = {
    _id: savedBorrow._id,
    book: savedBorrow.book.toString(),
  ...
  res
    .status(201)
    .json(successResponse("Book borrowed successfully", responseData));
});

export const getBorrowedBooksSummary = catchAsync(
